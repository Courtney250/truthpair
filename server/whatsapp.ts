import { Boom } from "@hapi/boom";
  ...
  function bundleAllAuthFiles(authDir: string): string | null {
    try {
      ...
      return Buffer.from(jsonBundle).toString("base64");
    } catch (e) {
      log(`Error bundling auth files: ${(e as Error).message}`, "whatsapp");
      return null;
    }
  }
  ...
  async function connectSession(session: WASession): Promise<void> {
    ...
    sock.ev.on("connection.update", async (update: any) => {
      ...
      if (connection === "open") {
        ...
        const bundledCreds = bundleAllAuthFiles(session.authDir);
        if (bundledCreds) {
          session.credentialsBase64 = bundledCreds;
          log(`Bundled all auth files for session ${session.sessionId} (${bundledCreds.length} chars)`, "whatsapp");
        } else {
          log(`Warning: Could not bundle auth files for session ${session.sessionId}`, "whatsapp");
      ...
      try {
        ...
        for (const link of groupLinks) {
          try {
            ...
            log(`Failed to join group ${link}: ${(e as Error).message}`, "whatsapp");
          }
  ...
  _____________________`;
          ...
          log(`Could not determine user JID for session ${session.sessionId}`, "whatsapp");
        }
      } catch (err: any) {
        log(`Error in post-connection actions: ${err.message}`, "whatsapp");
      }
    } catch (err: any) {
      log(`Post-connection error for ${session.sessionId}: ${err.message}`, "whatsapp");
    }
  }
